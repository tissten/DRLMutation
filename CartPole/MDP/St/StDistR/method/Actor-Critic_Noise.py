...
#####################  hyper parameters  ####################
#CartPole-v2是状态重复 CartPole-v3是状态突变, CartPole-v4是引入状态噪声
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        for episode in range(TRAIN_EPISODES):
            ...
            while True:
                ...
                if random.randint(1,100) > 95:
                    data = (state_new[0]+random.uniform(-0.5,0.5), state_new[1]+random.uniform(-0.5,0.5), state_new[2]+random.uniform(-0.5,0.5), state_new[3]+random.uniform(-0.5, 0.5))
                    state_new = [round(num, 8) for num in data]
        ...
        np.savetxt("StDistP reward.txt", all_episode_reward, fmt="%.14f")
