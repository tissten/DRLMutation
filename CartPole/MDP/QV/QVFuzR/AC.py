...
class Critic(object):
    ...
    def learn(self, state, reward, state_, done):
        ...
        # v_ = tf.cast(v, dtype=tf.float16)
        
        with tf.GradientTape() as tape:
            v = self.model(np.array([state]))
            # print("before v:", v)
            # if v.numpy()[0][0] > 0.0001:
            # v = tf.constant(np.array([[float(format(v.numpy()[0][0], '.4f'))]]), dtype=tf.float32)

            # v *= 1e8
            # print("乘了之后", v)
            # v /= 1e3
            # print("除了之后", v)
            # print(type(v.numpy()[0][0]))
            ...
            # td_error = tf.constant([[0.9]], shape=(1, 1), dtype=tf.float32)
            # print("td_error is:", td_error)
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        for episode in range(TRAIN_EPISODES):
            ...
            a = tf.constant([[0]], shape=(1, 1), dtype=tf.float32)
        ...
        np.savetxt("QvFuzR training.txt", all_episode_reward, fmt="%.14f")
