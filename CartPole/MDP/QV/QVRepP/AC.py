...
class Critic(object):
    ...
    def learn(self, state, reward, state_, pre_v, flag, done):
        d = 0 if done else 1
        ...
        with tf.GradientTape() as tape:
            ...
            if pre_v != 0 and flag % 10 == 0:
                v = tf.constant([[5]], shape=(1, 1), dtype=tf.float32)
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        for episode in range(TRAIN_EPISODES):
            ...
            pre_v = tf.constant([[0]], shape=(1, 1), dtype=tf.float32)
            flag = 1
            ...
            while True:
                ...
                try:
                    td_error = critic.learn(
                        state, reward, state_new, pre_v, flag, done
                    )  # learn Value-function : gradient = grad[r + lambda * V(s_new) - V(s)]
                    # print("outside pre_v:", pre_v)
        ...
        np.savetxt("QvRepP training.txt", all_episode_reward, fmt="%.14f")
