...
ALG_NAME = 'AC'
TRAIN_EPISODES = 7000  # number of overall episodes for training
...
if __name__ == '__main__':
    ...
    if args.train:
            ...
            while True:
                ...
                try:
                    if episode > 5000:
                        ...
                        # j = random.randint(0, len(state_pool)+1)
                        # del state_pool[j]
                        # del reward_pool[j]
                        # del state_new_pool[j]
                        # del action_pool[j]
                        if flag % 10 == 0:
                            i = random.randint(0, len(state_pool)+1)
                            state = state_new_pool[i]
                            reward = reward_pool[i]
                            state_new = state_pool[i]
                ...
                state_new_pool.append(state_new)
        ...
        np.savetxt("DsDisoP training.txt", all_episode_reward, fmt="%.14f")
