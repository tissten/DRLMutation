...
class Actor(object):
    ...
    def get_action(self, state, action_counts, greedy):
        _logits = self.model(np.array([state]))
        _probs = tf.nn.softmax(_logits).numpy()
        if greedy:
            # return np.argmax(_probs.ravel())
            # print(action_counts)
            # print("动作：", _probs.ravel())
            # print("动作维数：", len(_probs.ravel()))
            value = []
            for i in range(len(_probs.ravel())):
                value.append( _probs.ravel()[i] + np.sqrt(np.log(2) / action_counts[i]))
            # print(value)
    ...
    if args.train:
        ...
        for episode in range(TRAIN_EPISODES):
            ...
            while True:
                ...
                if flag % 10 == 0:
                    action = actor.get_action(state,counts,greedy=True)
                else:
                    action = actor.get_action(state,counts,greedy=False)
        ...
        np.savetxt("EmDisoP training.txt", all_episode_reward, fmt="%.14f")
