...
class Actor(object):
    ...
    def get_action(self, state, flag, greedy=False):
        _logits = self.model(np.array([state]))
        _probs = tf.nn.softmax(_logits).numpy()
        # print("before:", _probs.ravel())
        if flag % 10 == 0:
            _probs.ravel()[0] = float(format(_probs.ravel()[0], '.4f'))
            _probs.ravel()[1] = 1 - _probs.ravel()[0]
        # print("after:", _probs.ravel())
        if greedy:
    ...
    if args.train:
        ...
        for episode in range(TRAIN_EPISODES):
            ...
            while True:
                ...
                action = actor.get_action(state, flag)
        ...
        np.savetxt("PvFuzP training.txt", all_episode_reward, fmt="%.14f")
