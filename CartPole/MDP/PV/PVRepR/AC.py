...
class Actor(object):
    ...
    def get_action(self, state, flag, pre_probs, greedy=False):
        _logits = self.model(np.array([state]))
        _probs = tf.nn.softmax(_logits).numpy()
        # print("pre:", pre_probs)
        # print("now:", _probs.ravel())
        a = _probs.ravel()
        if pre_probs != [] and random.randint(1,100)>90:
            a = pre_probs
            # _probs.ravel() = pre_probs
        # print(flag)
        pre_probs = a
        # print("pre after :", pre_probs)
        if greedy:
            return np.argmax(_probs.ravel())
        return tl.rein.choice_action_by_probs(a), pre_probs  # sample according to probability distribution
    ...
    if args.train:
        ...
        for episode in range(TRAIN_EPISODES):
            ...
            while True:
                ...
                action, pro_probs = actor.get_action(state, flag, pro_probs)
                # print("outside:", pro_probs)
        ...
        np.savetxt("PvRepR training.txt", all_episode_reward, fmt="%.14f")
