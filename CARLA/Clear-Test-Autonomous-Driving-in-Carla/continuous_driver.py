...
def parse_args():
    ...
    parser.add_argument('--town', type=str, default="Town07", help='which town do you like?') #Town07
        ...
        if train:
            ...
            while a < 500:
                ...
                for t in range(args.episode_length):
                    ...
                    if current_ep_reward > 1500:
                        done =  True

                    # break; if the episode is over
                    if done:

                        if current_ep_reward < 1000:
                            score += 1
                        episode += 1
            ...
            print("Terminating the run.")
            steps = range(1, len(total_reward) + 1)
            plt.plot(steps, total_reward, marker='o', linestyle='-')
            plt.savefig(r'F:\ADS Carla\CARLA_0.9.8\WindowsNoEditor\Clear-Test-Autonomous-Driving-in-Carla\totral_reward.pdf', dpi=300, bbox_inches='tight', pad_inches=0.1)
            sys.exit()
