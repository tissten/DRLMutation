...
############# Training 模型训练        ##################################################
        ...
        pre_obs_t = torch.tensor([]) 
        pre_recurrent_hidden_states = torch.tensor([]) 
        pre_actions = torch.tensor([]) 
        pre_action_log_prob = torch.tensor([]) 
        pre_value = torch.tensor([]) 
        pre_rewards_t = torch.tensor([]) 
        ...
        for i_episode in range(num_updates):
            ...
            for timestep in range(num_steps):
                ...
                # if timestep != 0 and timestep % 10 == 0:
                #     rewards = reward
                # reward = rewards
                ...
                # print("obs is:", obs_t)
                # print("recurrent_hidden_states is:", recurrent_hidden_states)
                # print("actions is:", actions)
                # print("action_log_prob", action_log_prob)
                # print("value is:", value)
                # print("rewards_t is:", rewards_t)
                # print("masks is:", masks)
                if random.randint(1,100) > 90:
                    for j in range(len(obs_t)):
                        obs_t[j] = random.uniform(-1, 1)
                    # recurrent_hidden_states = 
                    actions = random.uniform(-1, 1)
                    action_log_prob = random.uniform(-1, 1)
                    value = random.uniform(-1, 1)
                    rewards_t = random.uniform(-1, 1)
                    # masks = random.choice(0, 1)
                    # print("1")
            ...
            agent.update(rollouts, timestep)
...
##############       Training模型保存     ###############################################################
        ...
        np.savetxt("SmRepP training.txt", scores_array, fmt="%.14f")
