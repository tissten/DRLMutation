...
class SActorCritic(nn.Module):
    def __init__(self):
        super(SActorCritic, self).__init__()
        self.affine = nn.Linear(8, 128)
        self.a_affine = nn.Linear(128, 128)
        self.b_affine = nn.Linear(128,128)
    ...
    def forward(self, state):
        ...
        state = F.relu(self.a_affine(state))
        state = F.relu(self.b_affine(state))
