...
class ActorCritic(nn.Module):
    ...
    def forward(self, state,flag = False):
        ...
        if flag:
            ...
            temp = (random.uniform(-modified_action_probs[0].item() / 2,
                                   (all_probs - modified_action_probs[0].item()) / 2) * 100)
            modified_action_probs[0] = torch.round(torch.tensor(temp)) / 100
            all_probs -= modified_action_probs[0]

            temp = (random.uniform(-modified_action_probs[1].item() / 2,
                                   (all_probs - modified_action_probs[1].item()) / 2) * 100)
            modified_action_probs[1] = torch.round(torch.tensor(temp)) / 100
            all_probs -= modified_action_probs[1]

            temp = (random.uniform(-modified_action_probs[2].item() / 2,
                                   (all_probs - modified_action_probs[2].item()) / 2) * 100)
            modified_action_probs[2] = torch.round(torch.tensor(temp)) / 100
