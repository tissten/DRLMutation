...
class ActorCritic(nn.Module):
    ...
    def forward(self, state,flag=False):
        ...
        if flag:
            pre_state_value=self.state_values.pop()
            self.state_values.append(state_value)
            self.state_values.append(pre_state_value)
        else:
            self.state_values.append(state_value)
    ...
    def selectaction(self, action_distribution, flag):
        if flag:
            ...
            pre_action = self.logprobs.pop()
            self.logprobs.append(action_distribution.log_prob(action))
            self.logprobs.append(pre_action)
        else:
            action = action_distribution.sample()
            self.logprobs.append(action_distribution.log_prob(action))
        return action.item()
    ...
    def calculateLoss(self, gamma):
        ...
        for logprob, value, reward in zip(self.logprobs, self.state_values, rewards_normalized):
            if flag % 20 == 0:
                logprob = random.choice(self.logprobs)
                value= random.choice(self.state_values)
                reward = random.choice(rewards_normalized)
