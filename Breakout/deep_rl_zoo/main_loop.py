...
class RewardTracker:
    def __init__(self, max_len=15):
        self.rewards = deque(maxlen=max_len)  # 最多存储15个元素

    def add_reward(self, reward):
        self.rewards.append(reward)  # 添加最新回合的奖励

    def get_average(self):
        if self.rewards:
            return np.mean(self.rewards)  # 计算平均奖励
        return 0  # 如果队列为空，则返回0
def run_env_loop_train(
        agent: types_lib.Agent, env: gym.Env,checkpoint: PyTorchCheckpoint
    ...
    while True:  # For each episode.
        ...
        if episodes>=15:
            tracker.add_reward(episode_reward)
            print(tracker.get_average())
            if tracker.get_average()>29:
                print(done)
                checkpoint.set_iteration(100)
                saved_ckpt = checkpoint.save()
                sys.exit()
        else : tracker.add_reward(episode_reward)
...
def run_env_loop(
    ...
    while True:  # For each episode.
        ...
        while z_s<z_s_m:
            ...
            yield env, timestep_t, agent, a_t
            seq = [3]
            observation, reward, done, info = env.step(random.choice(seq))
            z_s+=1
            episode_reward += reward
            first_step = False
            loss_life = False
        ...
        while True:  # For each step in the current episode.
            ...
            if done:  # Actual end of an episode
                ...
                print(e_step)
                logging.info('e_step: %f', e_step)
                print(episode_reward)
                logging.info('episode_reward: %f', episode_reward)
                all_reward+=episode_reward
                print(all_reward/episodes)
                logging.info('all_reward/episodes: %f %f', all_reward/episodes, episodes)
                if episode_reward > 50:
                    ok_episode+=1
                print(episodes, ok_episode)
                logging.info('episodes, ok_episode: %f %f', episodes, ok_episode)
...
def run_env_steps(num_steps: int, agent: types_lib.Agent, env: gym.Env, trackers: Iterable[Any]) -> Mapping[Text, float]:
    ...
    return stats
def run_env_steps_train(num_steps: int, agent: types_lib.Agent, env: gym.Env, trackers: Iterable[Any],checkpoint: PyTorchCheckpoint) -> Mapping[Text, float]:
    """Run some steps and return the statistics, this could be either training, evaluation, or testing steps.
    ...
    """
    seq = run_env_loop_train(agent, env,checkpoint)
    seq_truncated = itertools.islice(seq, num_steps)
    stats = trackers_lib.generate_statistics(trackers, seq_truncated)
    return stats
...
def run_single_thread_training_iterations(
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        # Run training steps.
        train_stats = run_env_steps_train(num_train_steps, train_agent, train_env, train_trackers,checkpoint)
        ...
        if should_run_evaluator is True:
            ...
            if eval_stats['mean_episode_return']>=50:
                print('done',iteration)
                checkpoint.set_iteration(101)
                saved_ckpt = checkpoint.save()
                sys.exit()
