...
class ActorCritic(types_lib.Agent):
    ...
    def act_p(self, timestep: types_lib.TimeStep) -> types_lib.Action:
        'Given timestep, return an action.'
        a_t = self._choose_action_p(timestep)
        return a_t
    ...
    @torch.no_grad()
    def _choose_action_p(self, timestep: types_lib.TimeStep) -> types_lib.Action:
    ...
    def _calc_loss(self, transitions: replay_lib.Transition) -> torch.Tensor:
        ...
        if self._step_t % 100 ==0:
            discount_t = (~done).float() * (self._discount+random.uniform(-0.1,0.01))
        else:
